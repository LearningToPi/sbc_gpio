...
if MODEL == 'ROCK5B':
    ...
    BUTTON_GPIO = (8*1) + 6
    BMX280_SPI_BUS = 1
    BMX280_SOFT_CS_CHIP = 3
    BMX280_SOFT_CS_PIN = (8*1) + 5
    DHT11_SPI_BUS = 0
    DHT11_SOFT_CS_CHIP = 1
    DHT11_SOFT_CS_PIN = (8*3) + 7
    I2C_DISPLAY_PORT = 7
    SER_1_DEV = '/dev/ttyS2'
    SER_2_DEV = '/dev/ttyUSB0'
## END Rock 5B
...
IR_SEND_SOCK = '/var/run/lirc/lircd1'
IR_RECV_SOCK = '/var/run/lirc/lircd0'
...
def run_tests():
    threads = []
    # init i2c Display
    display = I2cDisplay() if I2C_ENABLED else None
    

    # start the LED flashing
    if LED_ENABLED:
        new_thread = Thread(target=flash_LED, kwargs={'run_for': RUN_TIME, 'display': display})
        new_thread.start()
        threads.append(new_thread)

    # start the button press polling
    if BTN_ENABLED:
        new_thread = Thread(target=button_press_edge, kwargs={'run_for': RUN_TIME, 'display': display})
        new_thread.start()
        threads.append(new_thread)

    # start BME280 SPI polling
    if BMX_ENABLED:
        new_thread = Thread(target=sensor_bmx_280_spi, kwargs={'run_for': RUN_TIME, 'display': display})
        new_thread.start()
        threads.append(new_thread)

    # start DHT polling
    if DHT_ENABLED:
        new_thread = Thread(target=sensor_dht11_spi, kwargs={'run_for': RUN_TIME, 'display': display})
        new_thread.start()
        threads.append(new_thread)

    # start IR Transmitter
    if IR_ENABLED:
        new_thread = Thread(target=ir_transmit, kwargs={'run_for': RUN_TIME, 'display': display})
        new_thread.start()
        threads.append(new_thread)

    # start Serial
    if UART_ENABLED:
        new_thread = Thread(target=ser_send_receive, kwargs={'run_for': RUN_TIME, 'display': display})
        new_thread.start()
        threads.append(new_thread)
...
def flash_LED(run_for=30, display=None):
...
def button_press_polling(run_for=30, display=None):
...
def button_press_edge(run_for=30, display=None):
...
def sensor_bmx_280_spi(run_for=30, display=None):
...
def sensor_dht11_spi(run_for=30, display=None):
...
def ir_transmit(run_for=30, display=None, ir_timeout=1):
    ...
    try:
        # remove the devinput
        remotes.remove('devinput-32')
        remotes.remove('devinput-64')
    except:
        pass
    if len(remotes) <= 0:
        logger.error('No remotes found for LIRC.  Cancelling IR Transmit / Receive Test')
        test_bypass = True
    else:
        test_bypass = False
...
def ser_send_receive(run_for=30, display=None):
    ...
    while time() < stop_time:
        for baudrate in SER_BAUDRATES:
            for block_size in SER_BLOCK_SIZES:
                ...
                sleep((block_size * 8 / baudrate) + SER_SEND_DELAY_ADD)
